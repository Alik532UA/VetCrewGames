...
export default ts.config(
	...
	},
	{
		rules: {
			...
			// --- DEBUGGING-v8 § 4: заборона `console.*` у продакшн-коді.
			//
			// AGENTS.md називає це жорстким обмеженням («Console.log: Заборонено»)
			// із самого початку, а гейта під ним не було жодного — тобто правило
			// трималося на тому, що його прочитають. Це рівно те, що
			// CODE-QUALITY-v8 § 6.4.1 називає побажанням: текст без правила.
			//
			// Порушень на момент увімкнення нуль (поза `logService`, якому виняток
			// нижче), тож рівень одразу `error`, а не борг у `warn`: правило, у
			// якого нуль звернень, ставиться в `error` — саме такий конфіг тримає
			// стан, а не фіксує його.
			//
			// Перевірено зворотним експериментом (AI-AGENT-PITFALLS-v8 § 1.1):
			// `console.log` у `src/lib/services/analytics.ts` дає червоний `lint`,
			// без правила той самий рядок проходив мовчки.
			'no-console': 'error'
		}
	},
	{
		/**
		 * DEBUGGING-v8 § 4: єдиний виняток — сам логер.
		 *
		 * Він і є тим місцем, куди правило переадресовує всіх інших: `logService`
		 * дзеркалить записи в консоль розробника (`error` завжди, `warn`/`log`
		 * лише в dev) і відновлює буфер із `sessionStorage`, де рекурсивний виклик
		 * `this.error()` був би нескінченним.
		 *
		 * Виняток файловий, а не проєктний, — саме та «межа», якої § 6.4.1 вимагає
		 * поруч із причиною.
		 */
		files: ['src/lib/services/logService.svelte.ts'],
		rules: {
			'no-console': 'off'
		}
	},
	{
		/**
		 * `scripts/` — не продакшн-код, а інструменти командного рядка, і консоль
		 * там єдиний спосіб віддати результат.
		 *
		 * `check-build.mjs` друкує виміряні бюджети, `check-rules.mjs` — таблицю
		 * «дозволено / заборонено», `generate-sitemap.mjs` — кількість адрес. Саме
		 * ці рядки читає людина у виводі CI, коли гейт червоніє; відправити їх у
		 * `logService` неможливо — він браузерний і тягне за собою `$app/*`.
		 *
		 * Тому виняток названий межею («ці сім файлів»), а не знятий із правила
		 * взагалі: 26 звернень тут законні, у `src/` — жодного
		 * (CODE-QUALITY-v8 § 6.4.1 про причину й межу).
		 */
		files: ['scripts/**'],
		rules: {
			'no-console': 'off'
