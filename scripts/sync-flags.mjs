...
const EXCLUDED = {
	...
	/*
	 * XO і XC — самопроголошені утворення на окупованій території, обидва
	 * визнані лише країною-окупантом. Прибрані разом із `ru` тим самим рішенням
	 * автора, 2026-08-24.
	 *
	 * Технічно вони теж не з ISO: пакет бере їх із діапазону вільного
	 * призначення, тож `Intl.DisplayNames` назв для них не мав — у списку вони
	 * стояли сирими «XO» і «XC».
	 *
	 * ІНШІ безіменні коди пакета (`bq-bo`, `es-ct`, `gb-eng`, `gb-nir`,
	 * `gb-sct`, `gb-wls`, `xk`) лишаються НАВМИСНО: автор попросив прибрати
	 * рівно ці два. Косово (`xk`) назву має; решта — адміністративні одиниці
	 * визнаних держав, і питання до них інше, ніж до цих двох.
	 */
	xo: 'Південна Осетія — самопроголошене утворення, визнане лише окупантом',
	xc: 'Північний Кіпр — самопроголошене утворення, визнане лише окупантом'
};
