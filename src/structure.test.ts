 ...
 * (AI-AGENT-PITFALLS-v8 § 5.5). Міряється SLOC — рядки без коментарів і без
 * порожніх (`countSloc` нижче), а не `wc -l`: канон рахує складність логіки, а
 * не карає за вичерпну документацію (PROJECT-STRUCTURE-v9 § 7). Тут стояло
 * «`split('\n').length`, тобто на одиницю більше за `wc -l`» — опис першої
 * редакції, який пережив перехід на SLOC і почав описувати не ту величину, з
 * якою порівнюються числа в переліку нижче.
 *
...
const OVERSIZED_ALLOWLIST: Record<string, number> = {
	...
	//
	// 2026-08-23 (третя правка): 901 → 904. Рахунок на екрані підсумку показує
	// лише набране, максимум переїхав у підказку — три рядки розмітки й `maxHint`.
	// 2026-08-23: game-population 885 → 901. Підпис картки перестав розсувати
	// картку: внутрішній елемент для виміру в розмітці (обидва вжитки) плюс
	// правила `.game-card__name-text` зі станом переносу на змінних. Логіка при
	// цьому ПІШЛА з файлу — вимір і арифметика тепер у `$lib/utils/fitLabel.ts`
	// та `labelScale.ts`, тобто напрямок правильний, а число виросло через
	// розмітку й CSS.
	/*
	 * БОРГ ПЕРЕЇХАВ І ЗМЕНШИВСЯ, а не з'явився новий.
	 *
	 * Тут стояв `game-population/+page.svelte`: 904. Тепер маршрут має 78 SLOC —
	 * заголовок, лічильник раундів і картка підсумку, — а розкладка з правилами
	 * введення живе в цьому компоненті. Тобто те саме перевищення, тільке на
	 * 114 рядків менше й у файлі, який можна вставити в спільну вікторину.
	 *
	 * Заміряно, з чого воно складається: скрипт 227, розмітка 228, стилі 342.
	 * Найбільший окремий кусок — 136 рядків стилів картки (`.game-card*`), і саме
	 * вона повторюється в трьох місцях розмітки: слоти, панель джерела, результати.
	 *
	 * Наступний крок названий числом: винести картку компонентом (мінус ~180) і
	 * керування вводом — мишу, палець, клік, подвійний клік — у контролер
	 * (мінус ~227). Я цього НЕ зробив зараз свідомо: механічний переклад
...
describe('структура проєкту', () => {
	...
	/*
	 * ОСИРОТІЛІ ФАЙЛИ ПЕРЕЇХАЛИ У `module-reachability.test.ts`, а не зникли.
	 *
	 * Тут стояла перевірка, яка шукала базове ім'я файлу в тексті інших джерел.
	 * Канон v9 називає саме цей спосіб непридатним (§ 4.3.1, `PS-REACHABILITY`),
	 * і зворотний експеримент це підтвердив: на ланцюжку `__orphanA` → `__orphanB`
	 * і на `.ts`-сироті, згаданій лише в коментарі, вона давала нуль знахідок.
	 * Тримати обидві означало б лишити зелену галочку, яка обіцяє більше, ніж
	 * доводить, — а це найгірший різновид гейта.
	 */
	...
	/**
	 * `pushState`/`replaceState` НЕ оновлюють `page.url` — і це не помилка Kit.
	 *
	 * Поверхнева маршрутизація змінює `history` і `page.state`, а `page.url`
	 * навмисно лишає узгодженим із ЗАВАНТАЖЕНИМ маршрутом: інакше `page.url` і
	 * `page.data` розповідали б про різні адреси. У джерелі Kit 2.70.2 це видно
	 * прямо — `pushState` кладе в запис історії СТАРУ адресу
	 * (`[PAGE_URL_KEY]: page.url.href`) і присвоює лише `page.state`.
	 *
	 * ЦІНА ВЖЕ ЗАПЛАЧЕНА, і вона була повною. Обидві сторінки спільної гри писали
	 * код кімнати в адресу через `pushState`, а ефект «адреса — джерело правди»
	 * читав `page.url`. Тобто щойно кімната з'являлася, ефект бачив «кімнати в
	 * адресі немає» й одразу викидав із неї. Заміряно в браузері:
	 * `location.search` = `?room=99`, а `page.url` порожній — зайти в кімнату було
	 * НЕМОЖЛИВО ні в «Знайди пару», ні у вікторині. Тим же дефектом псувався QR:
	 * у код зашивалася адреса без `?room`, тобто сканування вело на порожню форму.
	 *
	 * Жоден гейт цього не бачив: типи сходяться, `svelte-check` мовчить, а
	 * помилка існує лише в рантаймі браузера.
	 *
	 * Тому крок в історії тут робить `goto` — справжня навігація, яка оновлює
	 * `page.url`. Якщо колись знадобиться саме поверхневий стан, читати його
	 * треба з `page.state`, а не з адреси, і тоді цей інваріант треба буде
	 * переписати разом із рішенням, а не обійти.
	 *
	 * Опція `goto(…, { replaceState: true })` тут НЕ при чому: ловиться імпорт із
	 * `$app/navigation`, а не слово.
	 */
	it('поверхнева маршрутизація не вживається (page.url від неї не оновлюється)', () => {
		const bad: string[] = [];
		for (const file of sources) {
			...
			for (const m of text.matchAll(/import\s*\{([^}]*)\}\s*from\s*'\$app\/navigation'/g)) {
				const named = m[1].split(',').map((part) => part.trim());
				for (const name of named) {
					if (name === 'pushState' || name === 'replaceState') bad.push(`${file}: ${name}`);
				}
			}
		}
		expect(
			bad,
			'ці функції змінюють адресу, але НЕ `page.url` — усе, що читає адресу ' +
				`як правду, побачить її в попередньому стані:\n${bad.join('\n')}`
		).toEqual([]);
	});
	...
	/**
	 * Сховище фонду чіпає ТІЛЬКИ контролер.
	 *
	 * Це не смак у шаруванні, а закріплений дефект. Сторінка вибору ділянки кликала
	 * `dropReserve()` напряму — стирала запис і лишала фонд у памʼяті синглтона.
	 * Кнопка «почати всі заново» через це не робила НІЧОГО: шапка показувала стару
	 * партію, а перший же запис повертав її й у сховище. Сам `reset()` при цьому був
	 * правильний, тобто жоден тест контролера цього й не міг зловити — ламалася
	 * ПРОВОДКА.
	 *
	 * Тому перевіряється саме вона: хто тримає стан, той і пише його в сховище.
	 */
	it('до збереження фонду ходить лише контролер', () => {
		const guilty = all
			.filter((f) => !isTest(f))
			.filter((f) => !f.includes('controllers/reserve.svelte.ts'))
			.filter((f) => !f.includes('services/reserveSave.ts'))
			.filter((f) => /from '[^']*reserveSave'/.test(read(f)));
		expect(
			guilty,
			`сховище фонду повз контролер:
${guilty.join(String.fromCharCode(10))}`
		).toEqual([]);
	});
