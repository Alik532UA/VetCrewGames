<script lang="ts">
	...
	/**
	 * Як часто оновлювати годинник для ЦИФРИ — відліку до початку партії в лобі.
	 *
	 * Секунда тут не про плавність: цифра й міняється раз на секунду, частіше
	 * оновлювати нічого.
	 */
	const CLOCK_MS = 1000;

	/**
	 * Як часто оновлювати годинник для СМУГИ часу ходу.
	 *
	 * Те саме число, що у вікторині (`ROUND_CLOCK_MS`), і з тієї самої причини: на
	 * секунді смуга рухалася б стрибками по відсотку своєї довжини, а на сто
	 * мілісекундах читається як час, що спливає.
	 */
	...
	/**
	 * Годинник — лише поки на нього дивляться, але тепер дивляться довше.
	 *
	 * ЩО ЗМІНИЛОСЯ. Доти умовою було `yieldReadyAt`, тобто «межу забрати чергу
	 * застосовно», і на власному ході інтервалу не було взагалі: секундний таймер,
	 * що цокає всю партію, був би витратою батареї на нічого. Тепер на екрані є
	 * смуга часу ходу, і вона йде в ОБОХ гравців — отже цокати треба, поки йде хід.
	 * «Ніщо» перестало бути ніщо.
	 *
	 * Частота взята від вікторини (`ROUND_CLOCK_MS`), і причина та сама: на секунді
	 * смуга рухалася б стрибками по відсотку, а на сто мілісекундах читається як
	 * час. Відлік у лобі — це цифра, і їй досить секунди.
	 */
	$effect(() => {
		...
		const turning = match?.turnEndsAt !== null && match?.turnEndsAt !== undefined;
		if (!browser || (!turning && !counting)) return;
		clock = Date.now();
		const timer = setInterval(() => (clock = Date.now()), turning ? TURN_CLOCK_MS : CLOCK_MS);
	 ...
	 */
	/**
	 * Скільки лишилося ПОТОЧНОМУ ХОДОВІ, мс. `null` — ходу немає.
	 *
	 * Тут стояв `yieldInMs` — «коли я зможу забрати чужу чергу», який на власному
	 * ході дорівнював `null`. Він і показувався рядком «Забрати хід можна за 0:43»,
	 * тобто зʼявлявся на початку КОЖНОГО чужого ходу й зникав на кожному своєму.
	 * Автор сказав про це прямо: «гра постійно стрибає через рядок».
	 *
	 * Тепер рахується сам хід, чий би він не був, і показується смугою — тією
	 * самою, що у вікторині. Смуга стоїть на місці й не зсуває дошку, а «скільки
	 * лишилося» видно з її довжини. Кнопки ж живуть окремо й унизу: вони
	 * зʼявляються, коли межа вийшла, і тоді на екрані справді є новина.
	 */
	const turnLeftMs = $derived.by(() => {
		const ends = match?.turnEndsAt;
		return ends === null || ends === undefined ? null : Math.max(0, ends - clock);
	});
