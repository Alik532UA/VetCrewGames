<script lang="ts">
	 ...
	 *
	 * Доти код кімнати писався в адресу `replaceState`, тобто без кроку в історії:
	 * «назад» вело на `/pairs/`, минаючи форму входу. Тепер вхід додає крок, і
	 * «назад» знімає `?room` — а разом із ним мусить зникнути й дошка.
	 *
	 * Без цього ефекту вийшло б найгірше з двох: адреса каже «я у формі входу», а
	 * на екрані партія. Тобто «назад» виглядало б як зламана кнопка.
	 *
	 * Інший код у адресі теж веде на форму: перейти з кімнати в кімнату однією
	 * зміною адреси нічим не безпечніше, ніж зайти в неї з форми, а розбирати й
	 * збирати підписки в одному такті — готове джерело привидів.
	 */
	$effect(() => {
		const wanted = roomFromUrl();
		if (match && wanted !== code) leaveRoom();
	});
	 ...
	 */
	/**
	 * Повне посилання на цю кімнату — для QR-коду в лобі.
	 *
	 * Береться з АДРЕСИ, а не складається з `base` і коду: адреса вже містить
	 * і мову, і префікс GitHub Pages, і `?room`. Складати те саме вручну
	 * означало б другу копію політики адрес — а вона в проєкті одна
	 * (`i18n/routing.ts`), і саме тому посилання не ламаються.
	 */
	...
	/**
	 * Записати код у адресу — КРОКОМ в історії, а не заміною, і через `goto`.
	 *
	 * Було `replaceState`, і «назад» із кімнати вело на `/pairs/` — тобто в меню
	 * розділу, ніби спільної гри й не було. Автор заміряв саме це. Крок в історії
	 * лишає в ній форму входу, тож «назад» веде туди, звідки прийшли: до списку
	 * кімнат. А оскільки крок є, адреса стала ДЖЕРЕЛОМ ПРАВДИ про те, у якій я
	 * кімнаті, — див. ефект нижче.
	 *
	 * ЦЕЙ КРОК РОБИВ `pushState`, І ВІН ЛАМАВ РЕЖИМ ЦІЛКОМ. Причина в його
	 * контракті: поверхнева маршрутизація змінює `history` і `page.state`, але
	 * `page.url` НЕ ПРИСВОЮЄ ніколи — навпаки, зберігає стару адресу в записі
	 * історії, щоб `page.url` лишався узгоджений із завантаженим маршрутом.
	 *
	 ...
	 * неможливо ні тут, ні у спільній вікторині.
	 *
	 * `goto` — справжня навігація: вона і додає крок в історію, і оновлює
	 * `page.url`. Тому джерело правди стало правдою, а не збігом. Заодно це
	 * виправило QR: `joinUrl` — це `page.url.href`, тобто доти в код зашивалася
	 * адреса БЕЗ `?room`, і сканування вело на порожню форму.
	 *
	 ...
	 *
	 * Розбирає рівно те, що збирає `enter`: підписки на кімнату, присутність і
