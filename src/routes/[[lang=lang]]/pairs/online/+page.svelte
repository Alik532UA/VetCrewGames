<script lang="ts">
	...
	/**
	 * Записати код у адресу — КРОКОМ в історії, а не заміною.
	 *
	 * Було `replaceState`, і «назад» із кімнати вело на `/pairs/` — тобто в меню
	 * розділу, ніби спільної гри й не було. Автор заміряв саме це.
	 *
	 * `pushState` лишає в історії форму входу, тож «назад» веде туди, звідки
	 * прийшли: до списку кімнат. А оскільки крок в історії тепер є, адреса стала
	 * ДЖЕРЕЛОМ ПРАВДИ про те, у якій я кімнаті, — див. ефект нижче. Без нього
	 * «назад» міняло б адресу, а дошка лишалася б на екрані.
	 */
