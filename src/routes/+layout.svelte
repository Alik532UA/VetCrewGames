<script lang="ts">
	...
	onMount(() => {
		...
		/*
		 * Пріоритет вибору мови: СЕГМЕНТ АДРЕСИ → `?lang=` → збережений вибір →
		 * типова (I18N-v8 § 3.3). Голий шлях означає «вибору в адресі не
		 * зроблено», тож саме там — і ТІЛЬКИ там — читаються два наступні
		 * джерела.
		 *
		 * `?lang=` з'явився заради сусідніх сайтів автора: вони передають мову,
		 * якою відвідувач читав ТАМ (`src/lib/siblings.ts` — та сама таблиця у
		 * восьми репозиторіях). Українську вони можуть назвати лише параметром,
		 * бо `/VetCrewGames/uk/` тут свідомо не існує; решту називають шляхом, і
		 * ті сюди приходять уже готовим сегментом.
		 *
		 * Тому параметр стоїть ВИЩЕ за збережений вибір, а не нижче. Інакше
		 * відвідувач, який колись обрав тут англійську, приходив би з української
		 * сторінки сусіда — і його б перекидало на англійську, тобто рівно та
		 * втрата мови, проти якої все це й зроблено. Параметр у сховище не
		 * пишеться: він каже про ЦЕЙ перехід, а не про вибір на цьому сайті.
		 *
		 ...
		 *
		 * `searchParams` читається саме тут, в `onMount`: під час prerender
		 * звернення до нього кидає й валить збірку (AGENTS.md, локальні пастки).
		 */
		if (!page.params.lang) {
			const asked = page.url.searchParams.get('lang');
			const chosen = (isLanguage(asked) ? asked : null) ?? settings.savedLocale();
			if (chosen && chosen !== DEFAULT_LANGUAGE) {
				goto(langPath(chosen, routeRestFromId(page.route.id)), {
					replaceState: true,
					noScroll: true
				});
			}
...
<!--
	...
	gzip зі стелі 110), і компонент із контролером його перевищили — заміряно
