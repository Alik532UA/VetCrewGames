...
const BACKED_BY_PARENT: Record<string, string> = {
	...
	// Рядок «хто я» (прапор, аватарка, імʼя, кубик) — вміст блоку форми входу
	// `.gate__panel--name`; винесений окремим файлом, коли туди додалася аватарка.
	// Підпис «Як вас звати?» свого фону не має, і саме його бачить ця перевірка.
	'src/lib/components/pairs/IdentityRow.svelte': 'рядок «хто я», фон дає .gate__panel',
	...
	// Вибір швидкості кімнати: дві смуги `SegmentedChoice` (їх ця перевірка не бачить
	// — компонент) і рядок-пояснення під шкалою раунду, коли вибрано «не обмежений».
	// Свого фону в пояснення немає, і саме його бачить ця перевірка. Компонент кладуть
	// ЛИШЕ в панель налаштувань лобі (`.lobby__panel--settings` в `OnlineLobby`, куди
	// його передає сніпетом `QuizLobby`), — і власний фон під рядком був би панеллю на
	// панелі, з видимим швом на межі. Той самий випадок, що в `SegmentedChoice` вище.
	'src/lib/components/quiz/QuizPacePicker.svelte':
		'вибір швидкості, фон дає .lobby__panel--settings',
	// Позначка «це ви» — це текст усередині `PlayerBadge`, і фон дає саме він
	// (пілюля з тлом і рамкою). Своїх правил у цього файлу більше немає зовсім:
	// вигляд спільний із позначкою «немає звʼязку», бо два однакові набори
	// розійшлися б. Перевірка бачить лише текстовий вузол, а не компонент навколо
	// нього, — звідси цей рядок.
	'src/lib/components/ui/YouTag.svelte': 'текст пілюлі, фон дає PlayerBadge',
	// QR має ВЛАСНЕ біле поле (сканеру потрібне саме воно), а підпис під ним лежить
	// на панелі запрошення `.lobby__panel--invite` у лобі — предку з іншого файлу.
	'src/lib/components/pairs/RoomQr.svelte': 'QR у лобі, фон дає .lobby__panel--invite',
	// Вибір країни — примітив із `ui/`, і кладуть його ЛИШЕ всередину панелі
	// (у формі входу — у блок «хто я»). Кнопка-тригер і випадна панель свій фон
	// мають; підпис над кнопкою — ні, і саме його бачить ця перевірка.
	'src/lib/components/ui/CountryPicker.svelte': 'вибір країни, фон дає панель-господар',
	// Вибір аватара — примітив із `ui/`, і кладуть його ЛИШЕ всередину панелі (у
	// формі профілю на `/account/` і в блоці «хто я» форми входу — `AvatarChooser`). Плитки свій колір мають; підписи двох груп
	// (`legend`) — ні, і саме їх бачить ця перевірка. Власний фон під підписом був
	// би панеллю на панелі — двома шарами того самого кольору з видимим швом.
	'src/lib/components/ui/AvatarPicker.svelte': 'вибір аватара, фон дає панель-господар',
	// Набір ігор кладуть у панель — у лобі це `.lobby__panel--settings`, у формі
	// входу він усередині фільтра, а той — у `.gate__panel`. Свій фон тут був би
	// панеллю на панелі: двома шарами того самого кольору зі швом на межі.
	'src/lib/components/quiz/QuizGamePicker.svelte': 'набір ігор, фон дає панель-господар',
	// Перелік кімнат вікторини малюється ЛИШЕ як пʼятий блок форми входу, всередині
	// `.gate__panel`, — тобто там же й з тієї самої причини, що `RoomList`, який він
	// обертає. Своя рамка у фільтра є, а фон дає та сама панель.
	'src/lib/components/quiz/QuizRooms.svelte': 'фільтр і список кімнат, фон дає .gate__panel',
	// Вибір «найняти / зробити самому» малюється ЛИШЕ у власному вікні
	// (`.care-window` в `ReserveGame`), яке має тло панелі й тінь. Свій фон тут був
	// би панеллю на панелі — двома шарами того самого кольору зі швом на межі.
	'src/lib/components/reserve/CareChoice.svelte': 'вміст вікна вибору, фон дає .care-window',
	// Перевірка «зробити самому» живе всередині того самого вікна — і більше ніде:
