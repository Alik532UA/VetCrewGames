 ...
 * ламає саме це.
 *
 * Виняток був один, і він не про смак: `<option>` не приймає розмітки взагалі —
 * браузер викидає будь-який вкладений тег, тож `formatFont` там не працював
 * фізично, і вибір стояв між `formatPlain` і неправильним шрифтом.
 *
 * СПИСОК ПОРОЖНІЙ З 2026-08-24. Єдиним записом був `ui/CountryPicker.svelte`, і
 * `<option>` звідти зник разом із нативним `<select>`: у власному меню пункт —
 * це `<button>`, який розмітку приймає. Тобто виняток закрився не послабленням
 * правила, а тим, що зникла причина.
 *
	...
	it('видимий текст зі словника проходить через formatFont', () => {
		...
		for (const file of components) {
			...
			const markup = source
				...
				.replace(/<!--[\s\S]*?-->/g, '')
				/*
				 * `.visually-hidden` — текст ДЛЯ МАШИНИ, і `formatFont` там не просто
				 * зайвий, а шкідливий: він міняє кириличну «і» на латинську, щоб літера
				 * була у шрифті. Того, що не малюється, це не стосується взагалі, зате
				 * скрінрідер вимовляє «Хiд» із латинською «i» покручем — і саме тому
				 * AGENTS.md забороняє формат для того, що читає машина.
				 *
				 * Вирізається цілий елемент разом із вмістом. Однойменний вкладений тег
				 * обрізав би блок зарано — тобто перевірка лишилася б суворішою, ніж
				 * треба, а не дірявою: помилка в безпечний бік.
				 */
				.replace(/<(\w+)[^>]*class="visually-hidden"[^>]*>[\s\S]*?<\/\1>/g, ' ');
