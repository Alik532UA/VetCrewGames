...
const RESOLVERS: Record<string, () => string> = {
	...
	/**
	 * Файли, яким § 7 (розмір) вибачений явним списком.
	 *
	 * Рядок про це протермінувався ДВІЧІ поспіль, і обидва рази однаково: борг
	 * закривали, а перелік у документі лишався старим. Спершу там стояло «три
	 * перевищення» з `GameHeader.svelte`, який уже вклався в межу; потім «два»
	 * з `game-population/+page.svelte` і `game-mythbusters/+page.svelte`, хоча
	 * обидва маршрути зі списку вийшли — логіка переїхала в контролери, а
	 * розкладка «Кого більше?» у `PopulationBoard.svelte`.
	 *
	 * Числа лишаються в самому `OVERSIZED_ALLOWLIST`: там їх стереже трійка
	 * перевірок «нових перевищень немає / список тільки скорочується / у списку
	 * немає тих, хто вже вклався». Гейта не мав саме ПЕРЕЛІК у документі — а
	 * читають спершу його, бо він каже, скільки боргу лишилося.
	 *
	 * Ключі витягуються з тексту тесту, а не імпортом: константа там локальна, і
	 * експортувати її заради перевірки означало б міняти форму гейта під
	 * перевірку. Коментарі знімаються про запас: усередині списку вони називають
	 * файли, яких у ньому саме НЕМАЄ, — сьогодні у зворотних лапках, тож збігу не
	 * було б і без цього кроку (заміряно: сирий блок і очищений дають той самий
	 * один ключ). Перші ж прямі лапки в коментарі зробили б різницю мовчки.
	 */
	'oversized-files'() {
		const source = read('src/structure.test.ts');
		const block = source.match(
			/const OVERSIZED_ALLOWLIST: Record<string, number> = \{([\s\S]*?)\n\};/
		)?.[1];
		expect(block, 'у structure.test.ts не знайдено OVERSIZED_ALLOWLIST').toBeDefined();
		const clean = (block as string).replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^\s*\/\/.*$/gm, ' ');
		const files = [...clean.matchAll(/'([^']+)'\s*:/g)].map((m) => m[1]).sort();
		return files.length === 0 ? 'немає' : files.join(', ');
	},

	/**
	 * Що САМЕ перевіряє Playwright.
	 *
	 * Тут стояло — у трьох місцях двох файлів одразу — «у `tests/` лежить лише
	 * `a11y.spec.ts`», «Playwright, рівно для axe», «сценарних E2E тут як не
	 * було, так і немає». Правдою це було чотири дні: зараз у `tests/` девʼять
	 * `*.spec.ts`, і серед них цілком сценарні (меню країн, підпис картки,
	 * чек-лист приймання форми входу).
	 *
	 * Чому рядок вартий гейта, а не просто правки. З «сценарних E2E немає»
	 * випливає протилежний висновок про роботу: «додати перевірку сюди — це
	 * завести новий клас тестів», хоча насправді це дописати файл у наявний
	 * прогін. Рівно на цій підставі перевірку й відкладають.
	 *
	 * Береться перелік імен, а не число: число росте й від перейменування, а
	 * імена кажуть, ЩО покрито. `identity.setup.ts` сюди не входить — це не
	 * перевірка застосунку, а проєкт `setup`, який звіряє, що на порті саме цей
	 * сайт; його стереже `src/test-runners.test.ts`.
	 */
	'e2e-specs'() {
		const specs = readdirSync('tests')
			...
			.sort();
		expect(specs.length, 'у tests/ немає жодного .spec.ts — перевірка осліпла').toBeGreaterThan(0);
		return specs.join(', ');
	},
	...
	/**
	 * Чи є в застосунку поля ПАРОЛЯ.
	 *
	 * Окремий рядок, а не уточнення до `text-inputs`, бо від нього залежить інша
	 * половина `FORM-INPUTS`: § 2 (показати/приховати), § 6 (CapsLock і чужа
	 * розкладка) і § 3.1 (полю пароля не дають «скопіювати»). Рядок «полів вводу
	 * немає» свого часу знімав ВЕСЬ файл; коли поля з'явилися, знявся весь файл
	 * — а розділ поруч далі стверджував, що паролів тут немає, і на цій підставі
	 * три параграфи лишалися «незастосовними» при готовому `PasswordInput`.
	 *
	 * Джерело істини — `autocomplete`, а не `type="password"`: тип у цьому
	 * компоненті виражений виразом (`type={shown ? 'text' : 'password'}`), тобто
	 * пошук по `type="password"` не знайшов би НІЧОГО саме тому, що показ пароля
	 * реалізовано. `autocomplete` натомість стоїть на кожному справжньому полі
	 * (цього окремо вимагає AUTH-FORM § 4) і в жодному іншому місці змісту не має.
	 */
	'password-fields'() {
		const components = walk('src').filter((f) => f.endsWith('.svelte'));
		expect(components.length, 'компонентів не знайдено — перевірка осліпла').toBeGreaterThan(0);
		const found = components.some((file) =>
			/autocomplete\s*=\s*"(?:current|new)-password"/.test(read(file))
		);
		return found ? 'є' : 'немає';
	},

	/**
	 * Чи заповнюють тут поля з буфера обміну.
	 *
	 * Від цього рядка залежить застосовність `INPUT-TOOLS` цілком. Тут стояло
	 * «жодне поле не заповнюють із буфера» — у файлі, який тим часом уже мав
	 * `ui/InputTools.svelte` із трьома кнопками й два поля `OnlineGate`, що ним
	 * користуються.
	 *
	 * Береться ВЖИВАННЯ, а не наявність файлу: компонент, який ніхто не малює,
	 * саме й означав би «кнопок у полях немає». Власний файл із перебору
	 * прибраний — інакше перевірка знаходила б сама себе.
	 */
	'clipboard-tools'() {
		const components = walk('src').filter(
			(f) => f.endsWith('.svelte') && !f.endsWith('ui/InputTools.svelte')
		);
		const used = components.some((file) => /<InputTools\b/.test(read(file)));
		return used ? 'є' : 'немає';
	}
};
