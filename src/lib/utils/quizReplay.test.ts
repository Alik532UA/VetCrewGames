...
/**
 * ЖУРНАЛ ПОПЕРЕДНЬОЇ КІМНАТИ ПІД ТИМ САМИМ КОДОМ (аудит 2026-09-25): чужий хід
 * `lead`, лишений під кодом, робив ведучим того, кого в кімнаті немає, і вікторина
 * стояла на раунді -1 назавжди. Правило бази такий журнал уже не пропускає; тут —
 * друга лінія.
 *
 * Зворотні експерименти: прибрати фільтр `createdAt` у пошуку першого `lead` або в
 * самому перепрогоні — червоніє.
 */
describe('журнал, старший за кімнату', () => {
	it('не рахується: ведучий — господар цієї кімнати, і його раунд іде', () => {
		const stale = snapshot([
			move('uid-old', 'lead', 500, { from: 'uid-old' }),
			move(HOST, 'round', 2000, { round: 0 })
		]);
		const log = replayQuizLog({ ...stale, info: { ...stale.info, createdAt: 1000 } });
