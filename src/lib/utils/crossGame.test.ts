...
describe('обвʼязка переїзду', () => {
	...
	/**
	 * ПЕРЕЇЗД ГРУПИ БЕЗ ВІКНА «ВАС ЗАПРОСИЛИ» (рішення автора 2026-09-26): ці люди
	 * хвилину тому грали разом і все про себе вже сказали.
	 *
	 * Зворотний експеримент: прибрати `move=1` з посилання — червоніє.
	 */
	it('посилання «перейти» каже, що це переїзд групи', () => {
		const next = new URL(crossGameLinks('uk', 'pairs', 'ab', '77').next ?? '', 'https://x.test');
		expect(next.searchParams.get('room')).toBe('77');
		expect(next.searchParams.get('move')).toBe('1');
	});
});
