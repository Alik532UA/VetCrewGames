...
function knownLocators(): Set<string> {
	...
	/*
	 * Локатор, переданий ЦІЛИМ пропом, а не склеєний із двох частин.
	 *
	 * `TimerBar` малює `data-testid={testId}`, а значення приходить літералом:
	 * `testId="quiz-round-progress"`. Такого локатора не бачив жоден із двох боків
	 * — у розмітці стоїть підстановка без шаблону (регексп вище ловить лише
	 * `{`…`}` з бектіками), а серед пропів значення вважалося тільки БАЗОЮ для
	 * шаблонів, що починаються з підстановки.
	 *
	 * Наслідок той самий, від якого застерігає докблок вище: пункт чеклиста, що
	 * назве СПРАВЖНІЙ локатор, падає — і автора тягне прибрати поле, а пункт без
	 * локатора не перевіряється взагалі. Заміряно на `quizonline_8`: смуга таймера
	 * переїхала у спільний компонент, і пункт про неї впав, хоч на сторінці вона
	 * та сама.
	 */
	if (/data-testid=\{(?:testId|scope)\}/.test(markup)) for (const base of bases) ids.add(base);
...
describe('чеклист бета-тестування', () => {
	...
	/**
	 * Кожен елемент керування ШАПКИ заявлений хоч одним пунктом.
	 *
	 * Пункт поза каноном, і додано його після дефекту, знайденого оком: при
	 * наведенні на кнопку повного екрана фон під шапкою ставав однотонною
	 * заливкою (`transform` на нащадку елемента з `backdrop-filter` ламає його
	 * backdrop root). Дефект жив, бо кнопки повного екрана не згадував ЖОДЕН
	 * пункт чеклиста.
	 *
	 * Чому § 5.1 цього не бачить, і не через недогляд канону: він вимагає, щоб
	 * кожен МАРШРУТ був заявлений вкладкою. Шапка стоїть на кожному маршруті, тож
	 * для неї ця умова виконана завжди — і мовчить однаково, є про неї пункти чи
	 * немає. Заявлення маршруту не є заявленням того, що на ньому намальовано.
	 *
	 * Межа саме по шапці, а не по всіх ~200 локаторах проєкту: шапка одна на всі
	 * екрани, її бачить кожен відвідувач на кожній сторінці, і елементів у ній
	 * одиниці. Вимагати пункт на кожен локатор означало б вимагати чеклист
	 * розміром із застосунок.
	 */
	it('кожен елемент керування шапки заявлений пунктом', () => {
		/*
		 * Джерело тут НЕ `LOCATORS`, і це важливо. Той набір навмисно є
		 * НАД-оцінкою: шаблон, що починається з `{testId}`, він розкриває кожною
		 * базою в проєкті, тож поруч із справжнім `header-theme-btn` там лежить і
		 * вигаданий `header-theme-play-again-btn`. Для питання «чи має пункт право
		 * назвати цей локатор» над-оцінка правильна; для питання «що МУСИТЬ бути
		 * заявлене» вона дала б двадцять шість фантомів.
		 *
		 * Тому беруться лише реальні елементи керування шапки: літеральні
		 * `data-testid="header-…"` плюс кнопка-тригер кожного `testId="header-…"`,
		 * бо `HeaderMenu` малює саме `{testId}-btn`.
		 */
		const literals = [...markup.matchAll(/data-testid="(header-[^"{}]*)"/g)].map((m) => m[1]);
		const menuTriggers = [...markup.matchAll(/\btestId="(header-[^"{}]*)"/g)].map(
			(m) => `${m[1]}-btn`
		);
		// Лише те, що НАТИСКАЮТЬ: `-value` і `-text` — це показники, не керування.
		const controls = [...new Set([...literals, ...menuTriggers])].filter((id) =>
			/-(btn|link)$/.test(id)
		);

		expect(
			controls.length,
			'елементів керування шапки не знайдено — перевірка мертва, змінився префікс'
		).toBeGreaterThan(3);

		const claimed = new Set(checks.map((check) => check.testid).filter(Boolean));
		const unclaimed = controls.filter((id) => !claimed.has(id));
		expect(
			unclaimed,
			`елемент шапки є на КОЖНОМУ екрані, і про нього немає жодного пункту: ${unclaimed.join(', ')}`
		).toEqual([]);
	});
	...
	/**
	 * § 3.4 `BETA-LEVEL-BALANCE`, і це сильніша умова за попередню.
	 *
	 * Один `manual`-пункт на вкладку рятує від найгіршого, але не від перекосу:
	 * контрольна група корисна доти, доки лишається групою, а не списком. Кожен
	 * `covered` витрачає час живої людини там, де автотест уже дивиться, — а в
	 * сусідньому проєкті цей перекіс дійшов до 26 `covered` проти 15 `manual`,
