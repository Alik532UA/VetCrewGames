...
describe('імена команди', () => {
	...
	/**
	 * Підставний перекладач: «Імʼя 1» … «Імʼя 16».
	 *
	 ...
	 *
	 * Перебираються ВСІ шістнадцять позицій генератора, а не випадкові: наївна
	 * реалізація «кинь випадковий» промахується рівно в одному кидку з
	 * шістнадцяти, тобто ймовірнісний тест був би зеленим у 94% прогонів — гірше
	...
	it('коли всі імена однакові, віддає його, а не порожнє', () => {
		const single = () => 'Єдиний Лікар';
		expect(randomCrewName(single, () => 0.5, 'Єдиний Лікар')).toBe('Єдиний Лікар');
	});
	...
	it('імена в межах однієї мови не повторюються', () => {
		...
		for (const [lang, dict] of Object.entries(LANGUAGES)) {
			...
			for (const key of keys) {
				...
				// «інше імʼя» виключає ЗНАЧЕННЯ, тож два однакових рядки — це п'ятнадцять
				// варіантів там, де в коді стоїть шістнадцять.
