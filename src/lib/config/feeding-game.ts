...
/**
 * Зображень страв ще немає — усі показують спільну заглушку.
 *
 * Коли автор їх намалює, тут міняється РІВНО ОДИН рядок:
 * `asset(`/images/food/${id}.webp`)`. Перелік того, що саме треба намалювати,
 * — у PROJECT-CONTEXT.md, розділ «Зображення, яких ще немає»; імена файлів
 * збігаються з `id` страви.
 */
const foodImage = (_id: string): string => asset('/images/placeholder.svg');
