/**
 * АВАТАР — КОРОТКИЙ РЯДОК `значок:колір`, а не картинка.
 *
 * ## Чому не зображення
 *
 * Зберігати його було б ніде: Storage у проєкті немає, а заводити його заради
 * підпису в списку — це і правила доступу до файлів, і межа розміру, і чистка
 * покинутого, і CSP на новий origin. У RTDB же лежать самі поля, і кожне з них
 * має межу: `payload` кімнати рахується в кілобайтах, а `members/$uid` пише
 * КОЖЕН вхід у кімнату. `cat:blue` важить дев’ять байтів і не додає жодного
 * нового механізму — ідея взята з сусіднього `Slovko`, де той самий вибір
 * записано рядком `internal:{значок}:{колір}` у полі `photoURL`.
 *
 * ## Чому два незалежні виміри, а не один список
 *
 * Чотирнадцять значків і вісім кольорів дають 112 різних підписів вісьмома
 * плюс чотирнадцятьма кнопками. Готовий список із 112 картинок займав би екран
 * прокруткою й не давав би нічого зверху.
 *
 * ## Значки — лише `lucide-svelte`
 *
 * Системні емодзі в UI заборонені (AGENTS.md): на Windows замість прапора
 * стоять дві літери, і те саме буває з тваринами. Набір повторює той, що автор
 * показав на знімку `Slovko`.
 *
 * ## Межа довжини тут і в правилах бази — ОДНЕ число
 *
 * `AVATAR_MAX` дублюється в `database.rules.json` (три вузли), і розійшовшись,
 * ці два числа дали б аватар, який видно на екрані й який база відкидає, —
 * тобто «не вдалося зайти в кімнату» без жодної причини. Найдовша пара зараз
 * `rabbit:violet` (13), а 24 лишає місце на довшу назву без перевикладання
 * правил. Саме межа й тримає запис обмеженим: взірець `[a-z]+:[a-z]+` сам собою
 * пропустив би мегабайт літер.
 */

/**
 * Значки. Порядок — той, у якому вони стоять у виборі, і він не випадковий:
 * спершу людина (типовий), далі тварини, далі знаки.
 */
export const AVATAR_ICONS = [
	'user',
	'cat',
	'dog',
	'rabbit',
	'bird',
	'fish',
	'snail',
	'turtle',
	'bug',
	'smile',
	'star',
	'heart',
	'zap',
	'target'
] as const;

/**
 * Кольори тла — ІМЕНАМИ, а не значеннями.
 *
 * У рядок їде імʼя (`cat:blue`), а не `#1d4ed8`, і причина не в довжині: колір
 * мусить лишитися тим самим після зміни палітри, і мусить бути перевірним. Самі
 * значення живуть токенами `--color-avatar-*` у `global.css`, тобто їхній
 * контраст із білим значком звіряє `src/contrast.test.ts` у всіх чотирьох темах,
 * а не чиєсь око.
 *
 * Палітра НЕ залежить від теми, і це рішення: аватар відрізняє людину, тож він
 * мусить виглядати однаково в мене й у неї. Видимість плитки на будь-якій
 * панелі дає тонка рамка з кольору тексту — той самий прийом, що у `Flag`.
 */
export const AVATAR_COLORS = [
	'red',
	'orange',
	'green',
	'teal',
	'blue',
	'violet',
	'pink',
	'slate'
] as const;

export type AvatarIcon = (typeof AVATAR_ICONS)[number];
export type AvatarColor = (typeof AVATAR_COLORS)[number];

/** Аватар, розібраний на дві частини. */
export interface AvatarLook {
	icon: AvatarIcon;
	color: AvatarColor;
}

/**
 * Типовий аватар. Він же відповідь на «поле відсутнє» і на «поле зіпсоване»:
 * порожньої плитки в списку не буває, бо порожнеча читалася б як дефект.
 */
export const DEFAULT_AVATAR = 'user:teal';

/** Межа довжини рядка. Те саме число стоїть у `database.rules.json`. */
export const AVATAR_MAX = 24;

/** Ключ сховища. Префікс `vetcrewgames_` додає фасад `storage`. */
export const AVATAR_KEY = 'pairs.avatar';

/** Взірець форми. Той самий стоїть у правилах бази. */
const SHAPE = /^[a-z]+:[a-z]+$/;

/**
 * Чи це наш аватар — тобто відома пара з відомих списків.
 *
 * Перевіряється НЕ лише форма: `dragon:gold` формі відповідає, а намалювати
 * його нічим. Невідоме значення означає або чужу руку, або наш власний дефект,
 * і в обох випадках правильна відповідь — типовий аватар, а не порожнє місце.
 */
export function isAvatar(value: unknown): value is string {
	if (typeof value !== 'string' || value.length > AVATAR_MAX || !SHAPE.test(value)) return false;
	const [icon, color] = value.split(':');
	return (
		(AVATAR_ICONS as readonly string[]).includes(icon) &&
		(AVATAR_COLORS as readonly string[]).includes(color)
	);
}

/**
 * ЧИ АВАТАР ВЛАСНИЙ — тобто вибраний людиною, а не типовий.
 *
 * Питання це не те саме, що `isAvatar`. Той відповідає «чи можна це намалювати»,
 * а тут — «чи є що показувати В РЯДКУ ІМЕНІ». Різниця видна в списку гравців:
 * типова плитка там стоїть у кожного, хто аватарки не вибирав, і не каже нічого —
 * вона лише розсуває прапор та імʼя. Автор попросив прямо: показувати лише те, що
 * відрізняється від типового.
 *
 * Тому «власний» — це і не порожньо, і не зіпсовано, і не `DEFAULT_AVATAR`. У
 * кімнату типовий і так не їде (`forRoom()` віддає `undefined`), але в базі є
 * записи зі старших збірок, а в профілі його можна вибрати руками — тобто
 * перевіряти саме значення однаково треба.
 */
export function isCustomAvatar(value: string | null | undefined): boolean {
	return isAvatar(value) && value !== DEFAULT_AVATAR;
}

/** Розібрати аватар. Невідоме — типовий, а не виняток: це підпис, а не стан гри. */
export function parseAvatar(value: string | null | undefined): AvatarLook {
	const [icon, color] = (isAvatar(value) ? value : DEFAULT_AVATAR).split(':');
	return { icon: icon as AvatarIcon, color: color as AvatarColor };
}

/** Зібрати аватар у рядок для запису. */
export function formatAvatar(icon: AvatarIcon, color: AvatarColor): string {
	return `${icon}:${color}`;
}
