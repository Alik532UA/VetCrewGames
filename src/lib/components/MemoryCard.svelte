...
<style>
	...
	.card--taken {
		opacity: 0.25;
		/*
		 * 80% РОЗМІРУ — прохання автора, і `transform` тут єдиний спосіб.
		 *
		 * Зменшити саму картку (`width`, `aspect-ratio`, `scale` як властивість
		 * розкладки) означало б зменшити КЛІТИНКУ сітки: сусідні картки поїхали б
		 * туди, де їх запамʼятали, — а місце на дошці і є те, що гравець тримає в
		 * голові. `transform` не входить у розкладку взагалі: клітинка лишається на
		 * місці, картка сідає в її середину.
		 *
		 * Зменшення йде тим самим повільним хвостом, що й згасання, і причина та сама:
		 * встигнути роздивитися, ЩО збіглося. Швидкий стрибок у 80% читався б як
		 * «картку прибрали».
		 */
		transform: scale(0.8);
		transition:
