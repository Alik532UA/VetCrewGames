...
<style>
	...
	.game-header__inner {
		 ...
		 * підлога в min-content заголовка (FLUID-SIZING-v8 § 1). Тут вона нічого
		 * не ламала лише тому, що середня колонка й так стискається до нуля;
		 * нуль записаний прямо, щоб наступний заголовок без `min-width: 0` не
		 * розсунув шапку мовчки.
		 */
	...
	/*
	 * РЯД, У ЯКОМУ Є КОМУ ПОСТУПИТИСЯ (FLUID-SIZING-v8 § 7).
	 *
	 * Сім кнопок по 36px — дві ліворуч, п'ять праворуч — займають рівно стільки,
	 * скільки дає 320 CSS px, і НІ ПІКСЕЛЯ БІЛЬШЕ. Заміряно на зібраному сайті
	 * (Playwright, вікно 320×640, `/account/`): колонки `76px 16px 196px` при
	 * 304 доступних, тобто запасу — 16px, і весь він у середній колонці, яку
	 * забирає перший же довший заголовок.
	 *
	 * Нижче цієї ширини запасу немає взагалі: на 280px ті самі 76 + 196 і
	 * проміжок 8 не вміщаються в 264, і сторінка віддає 16px ГОРИЗОНТАЛЬНОЇ
	 * ПРОКРУТКИ — заміряно на тринадцяти сторінках із чотирнадцяти, остання
	 * кнопка ряду виїжджає за екран.
	 *
	 * Поступитися доти не було кому: обидві групи — флекс-ряди без переносу, а
	 * колонка `auto` не стискається нижче min-content. Це рівно § 7: «у
	 * флекс-рядку не може бути `flex-shrink: 0` на ВСІХ дітях — поступитися нема
	 * кому, і рядок віддає горизонтальну прокрутку».
	 *
	 * Перенос ряду — те, що § 7 приписує другим кроком (стиснути шрифт →
	 * перенести рядок → трикрапка). Зменшити кнопки не можна: 36px — уже
	 * домовлений мінімум цього щільного ряду (`tests/touch-targets.spec.ts`).
	 *
	 * Ціна нульова там, де все й так вміщається. Заміряно з переносом і без
	 * нього в тому самому вікні: на 320px обидва варіанти дають ті самі
	 * `76px 16px 196px`, ту саму висоту шапки 44px і нуль прокрутки — тобто
	 * вище 296px не змінюється НІЧОГО. На 280px шапка стає 84px замість
	 * горизонтальної прокрутки всієї сторінки.
	 *
	 * Критерій, про який ідеться, — WCAG 2.2 AA 1.4.10 Reflow, і 320 CSS px у
	 * ньому не про рідкісний телефон: ту саму ширину дає 1280px під 400% зумом.
	 * Тримає це `tests/reflow.spec.ts`.
	 */
	.game-header__right {
		display: flex;
		align-items: center;
