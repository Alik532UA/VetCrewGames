<script lang="ts">
	 ...
	 *
	 * Сторінка тримає ще й навігацію, лічильник раундів і `GameOverCard` — усе,
	 * чого в спільній вікторині бути не мусить: там раунд один, а підсумок живе в
	 * кімнаті. Через відсутність такого поділу цю гру доти не можна було внести в
	 * `ONLINE_GAMES`; у маршруті лежало 374 рядки, з яких дошка — сімдесят сім.
	 *
	 * Це той самий поділ, що вже є в `MythCard`, `FeedingBoard` і `HabitatBoard`.
	 *
...
<style>
	...
	.cards-grid {
		/*
		 * ВИСОТА КАРТИНКИ КАРТКИ — оголошена ТУТ, бо тут вона й вживається.
		 *
		 * Раніше вона стояла на `.game-page` у маршруті, тобто в іншому файлі. У
		 * рантаймі це працювало (предок), але означало, що дошка, вставлена деінде,
		 * тихо втрачає розміри: `var()` без запасного значення робить оголошення
		 * недійсним, і `max-width` зникає ЦІЛКОМ. У спільній вікторині це б і
		 * сталося — там предка `.game-page` немає.
		 *
		 * `svh`, а не `dvh`: динамічна висота на телефоні МІНЯЄТЬСЯ, коли ховається
		 * адресний рядок, і розкладка повзе разом із нею.
		 */
