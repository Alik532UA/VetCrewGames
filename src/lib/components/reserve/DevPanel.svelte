...
<style>
	...
	.dev__grid {
		display: grid;
		/*
		 * `min(9rem, 100%)`, а не гола `9rem` — та сама причина, що в
		 * `BiomePicker` (FLUID-SIZING-v8 § 1.1): 9rem мусить лишатися порогом
		 * переносу, а не підлогою ширини.
		 *
		 * Тут воно ще нікого не зачепило — панель службова, і 144px вміщаються
		 * навіть у 280px. Правиться разом із сусідом саме тому: обидві форми
		 * однакові, і лишити одну означало б лишити зразок, з якого наступну
		 * сітку спишуть у поламаному вигляді.
		 */
