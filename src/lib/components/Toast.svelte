...
<style>
	/*
	 * ЗВЕРХУ ПРАВОРУЧ, а не знизу на всю ширину.
	 *
	 * Прохання автора зʼявлялося двічі й змінилося: спершу «зверху ліворуч
	 * екрану», потім — праворуч. Тому кут тут описаний як РІШЕННЯ, а не як смак,
	 * і причини лишаються ті самі, що були:
	 *
	 *  * НЕ ВНИЗУ: сповіщення заповідника розповідають про те, що сталося з
	 *    твариною, а очі в цей час на карті — у центрі й нижче. Смуга внизу
	 *    перекривала саме те, про що йшлося;
	 *  * НЕ НА ВСЮ ШИРИНУ: рядок на весь екран забирав місце незалежно від
	 *    довжини тексту;
	 *  * САМЕ ПРАВОРУЧ: зверху ліворуч у заповіднику стоїть панель показників, а
	 *    праворуч після переїзду керування часом униз не стоїть нічого.
	 *
	 ...
	 *
	 * `left: auto` обовʼязковий: без нього тости лишилися б розтягнутими, бо
	 * ширину їм задавав саме розпір між `left` і `right`.
	 *
	 * ВІДСТУП ПРАВОРУЧ БІЛЬШИЙ ЗА ЛІВИЙ, і це не окомір. Праворуч живе власна
	 * смуга прокрутки (`PageScrollbar`): 10px у спокої, 20px під курсором. Тост
	 * має `z-index: 10000` проти її 8000, тобто накрив би її — і поки він на
	 * екрані, за смугу не можна було б потягнути. `--space-lg` (24px) лишає її
	 * повністю вільною навіть у наведеному стані.
	 *
	 * `--toast-top` — ЗСУВ, який може попросити сторінка. Потрібен він рівно
	 * одному екрану: у заповіднику панель показників на вузькому вікні
	 * переноситься в кілька рядів і доходить до правого краю, тобто тост ліг би
	 * на числа, про зміну яких говорить. Панель міряє себе сама (`ReserveHud`),
	 * бо на телефоні вона втричі вища, ніж на широкому екрані. Нуль за
	 * замовчуванням означає «нічого не зсуваємо».
	 */
	.toasts {
		position: fixed;
	...
	.toast {
		 ...
		 *
		 * Відніманню відповідають ДВА різні відступи: правий тепер `--space-lg`
		 * (обходить смугу прокрутки), лівий лишається `--space-md`. Разом вони й
		 * дають межу, за яку тост не вилізе на 320px.
		 */
		width: min(420px, calc(100vw - var(--space-md) - var(--space-lg)));
