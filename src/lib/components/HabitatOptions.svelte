...
<style>
	/*
	 * `min(130px, 100%)`, а не гола довжина: інакше 130px стають ПІДЛОГОЮ
	 * ширини колонки, і на екрані 320px сітка розпирає сторінку
	 * (FLUID-SIZING-v8 § 1.1).
	 *
	 * 130, а не 150, і різниця тут не косметична. Поріг переносу — це не сама
	 * колонка, а колонка ПЛЮС проміжок: дві по 150 з проміжком 8 просять 308,
	 * а сторінці на екрані 320px дістається 304. Трьох пікселів забракло — і
	 * сім варіантів ставали одним стовпцем заввишки 431px при 406 видимих.
	 * Заміряно; з 130 дві колонки влазять із запасом і висота падає вдвічі.
	 */
	.options {
		display: grid;
