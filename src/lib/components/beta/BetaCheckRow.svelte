...
<style>
	 ...
	 *
	 * ВИБРАНИЙ ГОЛОС — НЕ `.seg-item--on`, і це єдине місце, де смуга відходить від
	 * акценту. Акцент тут збрехав би: «Не працює», залите тим самим помаранчевим,
	 * що й «Працює», читалося б як ще одна добра позначка, а колір стану — рівно те,
	 * за чим тестувальник очима шукає зламане.
	 *
	 * Стан позначено НЕ лише кольором: кільце всередині сегмента, напівжирний
	 * підпис і `aria-pressed` на кнопці. Кольору самого замало й для того, хто його
	 * не розрізняє, і для скрінрідера.
	 *
	 * ПІДПИС ЛИШАЄТЬСЯ КОЛЬОРОМ ТЕКСТУ (`--color-text-on-panel` сегмента), і це
	 * виправлення, а не спрощення. Доти
	 ...
	 *
	 * Тому колір стану пішов туди, де він не мусить бути читним ТЕКСТОМ: у кільце
	 * й у слабку підкладку. Обидва — з токенів теми, тож у темній темі стан
	 * червоний тим червоним, який має ця тема.
	 *
	 * Чому 1.4.11 (3:1 для елементів керування) тут не порушено при
	 * `--color-warning` 2,66 на кільці: колір НЕ є єдиним носієм стану. Кільце,
	 * напівжирність і `aria-pressed` кажуть те саме, а самі підписи кнопок —
	 * «Не працює», «Дивно», «Працює» — називають стан словами.
	 *
	 * Чому цього не зловив `src/contrast.test.ts`: тло сегмента прозоре, тож пара
	 * «тло+текст» статично не розвʼязується й іде в НЕПОКРИТО. Саме тому те число
	 * друкується у звіті — ось як виглядає дефект, що в ньому сховався.
	 *
	 * Кільце — `box-shadow` усередину, а не рамка: у сегмента рамки немає, і рамка,
	 * що зʼявляється лише на вибраному, штовхала б сусідів на два пікселі.
	 */
	.vote--active {
		font-weight: 700;
	}
