...
<div class="board">
	<!--
		...
		ГІЛКИ «ПАРТІЯ СКІНЧИЛАСЯ» ТУТ НЕМАЄ, і це прохання автора: «написаний
		результат партії "Нічия" — це означає, що зверху дублююча зайва інформація».
		Так і було: рядок казав, ЩО партія скінчилася, а на два блоки нижче стояло
		«Нічия» разом із кнопкою «Зіграти ще». Друге містить перше — і містить із
		подробицями.

		Через це `role="status"` переїхав на підсумок. Доти кінець партії оголошував
		скрінрідеру САМЕ цей рядок (підсумок ролі не мав), тож без переїзду
		найважливіша подія партії перестала б оголошуватися взагалі — тихо, бо живих
		регіонів не рахує жоден гейт.
	-->
	{#if !match.over}
	...
	<!--
		...
		ЩО ЗМІНИЛОСЯ І ЧОМУ. Панель більше не називає, хто зник: про це каже пілюля
		«немає звʼязку» біля самого імені в таблі нижче. Прохання автора: «знизу вже
		написано… це значить, що зверху панель немає сенсу». І рація тут сильніша,
		ніж на знімку: після партії `actor` дорівнює `null`, тобто межу «забрати хід»
		рахувати нема від чого — ні кнопок, ні відліку не лишається, — і панель
		вироджувалася в переказ пілюлі. А закрити вкладку, догравши, робить майже
		кожен, тож ставалося це в БІЛЬШОСТІ доіграних партій, а не в кутовому разі.

		Тому умова показу тепер — сама дія: чужий хід іде й колись його можна буде
		забрати (`onYield || countdown` — це і є «межа застосовна»). Немає чого
		робити — немає панелі. Разом із цим пішла гілка «зараз ваш хід — грайте
		далі»: рядок черги вище вже каже «Ваш хід», а пілюля каже, що суперника
		немає.
	-->
	{#if !match.over && (onYield || countdown)}
		<div class="board__stall text-panel" role="status" data-testid="pairs-stall-panel">
			{#if away.length === 0}
				<!--
					Присутність є, а ходу немає: людина сидить у вкладці й не грає. Пілюлі в
					таблі в цьому разі НЕМА (вона від присутності), тож причину каже панель —
					інакше відлік стояв би без жодного пояснення.
				-->
