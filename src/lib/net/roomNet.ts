 ...
 * памʼяті.
 *
 * НЕ ВСЕ: три дороги йдуть повз нього, і це названо, а не сховано (аудит
 * 2026-09-25). Оголошення переїзду в іншу гру пише в стару кімнату
 * `RoomPlace.announce` → `utils/crossGame` → `rtdbRoom.announceNext`; своя покинута
 * кімната закривається зі списку `LobbyFeed.close`; вихід назовсім зі смуги
 * «вас чекають» — `net/leave.ts`. Жодна з них не частина сесії кімнати, тож і
...
export interface RoomNet {
	...
	/** Серцебиття кімнати, поки вона на екрані, — транспортом самої кімнати. */
	beat(transport: RoomTransport): () => void;
...
export const liveNet: RoomNet = {
	...
	beat: (transport) => startRoomBeat(transport),
