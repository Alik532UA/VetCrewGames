 ...
 * перебір щось дав, він мусить влучити у ВІКНО життя конкретної кімнати. Це
 * вікно тепер ширше, ніж записано в рішенні, і це названо (аудит 2026-09-26):
 * порожня кімната НЕ зникає разом із вкладкою господаря (домовленість
 * `holdRoom` прибрано 2026-09-24, бо перезавантаження вбивало кімнату), тож
 * покинута живе до шести годин тиші плюс доби прибиральника, а найдовше — дві
 * доби віку (`scripts/sweep-plan.mjs`). І в розпочату вікторину новачок
 * заходить гравцем, а не глядачем; глядачем — лише в «Знайди пару».
 *
...
export async function roomTransport(code: string): Promise<RoomTransport> {
	...
	return {
		...
		async restart(seed, roster) {
			 ...
			 *
			 * НОВА ПАРТІЯ — НОВИЙ `startedAt`, і тим самим записом. Доти тут писалися
			 * лише зерно й журнал, а позначка початку лишалася від ПЕРШОЇ партії. Від неї
			 * рахується очікування першого ходу (`turnLimit.ts`), тож після партії,
			 * довшої за межу, «Зіграти ще» віддавало дошку, на якій суперник МИТТЄВО
			 * міг забрати чергу або завершити партію. Тести цього не бачили: підставний
...
/**
 * ОГОЛОСИТИ ПЕРЕЇЗД: у кімнаті `code` тепер лежить код кімнати `nextCode`.
 *
 * Прохання автора: «господар створює кімнату іншої гри, а решті в старій кімнаті
 * зʼявляється кнопка „перейти“ з її кодом». Без цього після фіналу група
 * розсипається — кожен опиняється в переліку кімнат і шукає одне одного заново.
 *
 * ## Чому окрема функція, а не метод транспорту
 *
 * Транспорт — це «моя кімната»: підписка, ходи, стан. А тут запис у ЧУЖУ кімнату:
 * нову створює сторінка іншої гри, і саме вона мусить сказати старій, куди йти.
 * Метод у транспорті означав би або другий транспорт на час одного запису, або
 * поле «а ще ця кімната» — обидва гірші за одну функцію з двома кодами.
 *
 * Прав це не додає: правило `info` дозволяє запис лише господареві, і саме він тут
 * і пише. Не кидає, бо переїзд — зручність: партія, яка щойно скінчилася, не
 * повинна ламатися через невдалий запис довідки.
 */
export async function announceNext(code: string, nextCode: string): Promise<void> {
	try {
		const { db } = await connect();
		const { ref, set } = await import('firebase/database');
		await set(ref(db, `rooms/${code}/info/nextCode`), nextCode);
	} catch (error) {
		logService.warn('network', 'next room not announced', { code, reason: String(error) });
	}
}
