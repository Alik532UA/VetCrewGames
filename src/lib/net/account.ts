 ...
 *
 * Навіщо окремо від `saveProfile`. Імʼя міняють не тільки у формі профілю: на
 * екрані входу в кімнату стоїть те саме імʼя, і зміна там мусить доїхати в
 * акаунт (прохання автора: «якщо поміняти імʼя в онлайн грі, то воно
 * підтягується в профайл»). Через `saveProfile` це означало б відправити заразом
 * @нік і країну — поля, яких на тому екрані немає й значень яких сторінка лобі
 * не знає, — і заплатити перевіркою вільності псевдоніма за зміну підпису.
 *
 * ВИКЛИКАЄТЬСЯ ЛИШЕ ДЛЯ НАЯВНОГО ПРОФІЛЮ, і межу тримає той, хто кличе — рівно з
 * тієї самої причини, що в `saveAvatar`: `.validate` батьківського вузла
 * (`hasChildren(['name','handle'])`) при записі в дитину не переоцінюється, тож
 * цей виклик на порожньому місці створив би профіль з одного імені, без @ніка.
 */
export async function saveName(name: string): Promise<void> {
	const { uid, db } = await connect();
	const { ref, set } = await import('firebase/database');
	await set(ref(db, `users/${uid}/profile/name`), name);
}
