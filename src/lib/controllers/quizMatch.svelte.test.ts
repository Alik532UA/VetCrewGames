...
describe('той, хто зник із кімнати', () => {
	...
	/**
	 * СЕБЕ ЗНИКЛИМ НЕ ПОКАЗУЄ (аудит 2026-09-24). Без звʼязку SDK сам виконує мій
	 * `onDisconnect` у місцевій копії — і доти я бачив на весь екран «Чекаємо: <я>»,
	 * а господар за пʼятнадцять секунд отримував кнопку «прибрати» навпроти себе.
	 * Пауза при цьому рахує й мене: партія стоїть і в мене, поки мене немає.
	 *
	 * Зворотний експеримент: повернути в `awayOthers` увесь `away` — червоніє.
	 */
	it('себе зниклим не показує, але чекання рахує й мене', async () => {
		const { room, host, stop } = table();
		host.present = [GUEST];
		await host.startRound(0);

		expect(host.away.map((member) => member.uid)).toEqual([HOST]);
		expect(host.awayOthers).toEqual([]);
		room.tick(0);
		stop();
	});
	/**
