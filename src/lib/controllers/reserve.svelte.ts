...
/**
 * Скільки діб не перепитувати про ту саму тварину після «нічого не робити».
 *
 * Без витримки відмова означала б те саме питання наступної доби — і так щодня,
 * поки тварина не помре. Пʼять діб — той самий порядок, що терпіння браконьєрів:
 * досить, щоб не докучати, замало, щоб забути про тварину зовсім.
 */
const ASK_COOLDOWN_DAYS = 5;
...
export class ReserveController {
	...
	/**
	 * Питання, на яке гра чекає відповіді. `null` — питання немає.
	 *
	 * Поки воно тут, час спинений (`speed = 0`): рішення «лікувати самому чи
	 * найняти» не мусить ухвалюватися під тиском доби, яка йде далі.
	 */
	pending = $state<Extract<ReserveEvent, { kind: 'needs-care' }> | null>(null);

	/** Швидкість, з якою грали до питання. Повертається разом із відповіддю. */
	#speedBefore: Speed = 1;

	/**
	 * Кого й коли вже питали: щоб «нічого не робити» не питали щодня.
	 *
	 * Звичайний обʼєкт, а не `Map`: реактивність тут не потрібна (розмітка це
	 * значення не читає), а `Map` у рунічному файлі проєкт забороняє — інакше
	 * довелося б брати `SvelteMap` і платити реактивністю за пам'ять, яку ніхто
	 * не показує.
	 */
	#askedOn: Record<number, number> = {};
	...
	#announce(event: ReserveEvent): void {
		...
		if (event.kind === 'needs-care') this.#ask(event);
		this.onEvent?.(event);
	}

	/**
	 * ПИТАННЯ ДО ГРАВЦЯ: дію потрібно зробити, а працівника немає.
	 *
	 * ## Чому черги немає, а є одне питання
	 *
	 * Доба може оголосити потребу для кожної тварини — на десяти тваринах це десять
	 * вікон підряд, і жодне з них не буде прочитане. Тому питання ОДНЕ: перше, що
	 * прийшло цієї доби; решта дістанеться наступним добам. Порядок при цьому не
	 * випадковий — `siteDay` перебирає тварин у фіксованому порядку, тобто спершу
	 * питають про тих, хто в списку перший.
	 *
	 * ## Витримка після «нічого не робити»
	 *
	 * Без неї відмова означала б те саме питання наступної доби — і так щодня, поки
	 * тварина не помре. Пʼять діб — це той самий порядок, що терпіння браконьєрів:
	 * досить, щоб не докучати, замало, щоб забути.
	 *
	 * Пам'ять про питання живе ТУТ, а не в сейві: після перезавантаження гра спитає
	 * знову, і це правильніше за міграцію формату заради чемності.
	 */
	#ask(need: Extract<ReserveEvent, { kind: 'needs-care' }>): void {
		if (this.pending) return;
		const asked = this.#askedOn[need.animalId] ?? -Infinity;
		if (this.day - asked < ASK_COOLDOWN_DAYS) return;

		this.pending = need;
		/*
		 * ЧАС СПИНЯЄТЬСЯ, поки питання на екрані. Швидкість запамʼятовується, щоб
		 * повернути саму ту, з якою грали: інакше кожне питання скидало б ×5 на ×1.
		 */
	...
	/**
	 * Відповідь на питання: найняли, зробили самі або нічого не робимо.
	 *
	 * `null` для `outcome` означає «нічого не робимо» — і саме тоді ставиться
	 * витримка. Найм і «зробив сам» витримки не ставлять: там дія відбулася, і
	 * наступне питання прийде лише якщо вона знову буде потрібна.
	 */
	answer(outcome: 'hired' | 'self' | 'ignored'): void {
		const need = this.pending;
		if (!need) return;

		if (outcome === 'ignored') this.#askedOn[need.animalId] = this.day;
		this.pending = null;
