...
import { getNextQuestion, questionById, type GameQuestion } from '$lib/config/myth-game';
...
export class MythGameController {
	...
	#next(): void {
		...
		let question =
			(this.#pick !== undefined && this.#usedThisGame.length === 0
				? questionById(this.#pick)
				: null) ?? getNextQuestion([...this.#usedThisGame, ...this.#usedEver], this.#random);
	...
	#persistUsed(): void {
		/*
		 * У ПАРТІЇ ІЗ ЗЕРНОМ — НЕ ПИСАТИ (аудит 2026-09-26). Спільна партія памʼять
		 * «показане назавжди» не читає (`start`), але доти писала: `#usedEver` там
		 * порожній, тож кожен раунд онлайн-вікторини й кожен крок перевірки в
		 * заповіднику перезаписував соло-памʼять ОДНИМ питанням — і захист від повторів
		 * у соло обнулявся після будь-якої гри онлайн.
		 */
