 ...
 * раунд починається одним ходом ведучого (спершу господар, після перехоплення —
 * той, хто підхопив, `leader`), і від цієї позначки кожен рахує свій дедлайн.
 *
...
export class QuizMatch {
	 ...
	 * потрібна — а редакція правил ГРИ потрібна, див. `QUIZ_RULES_VERSION` у
	 * `config/roomRules.ts`.
	 */
	 ...
	 *
	 * Не з журналу й не зі складу кімнати: рядок у `members` сам не гасне (його
	 * прибирає лише вихід назовсім або господар), а `presence` гасне сама —
	...
	/**
	 * Номер поточного раунду — найбільший, який оголосив ведучий.
	 *
	 * `-1` означає «партія почалася, але жодного раунду ще не оголошено». Це
	 * законний стан, а не помилка: ведучий пише перший раунд окремим ходом, і між
	 ...
	 * закрив вкладку, лишається в `members` (рядок сам не гасне), відповіді від
	 ...
	 * перевіряла ще й статус. Тепер статус `over` господар пише сам, щойно партію
	 * дограно (`roomPolicies`), — тож два читання мусили стати одним: інакше вони
	 * розійшлися б тихо, бо кожне виглядає правильним окремо.
	 */
	get over(): boolean {
