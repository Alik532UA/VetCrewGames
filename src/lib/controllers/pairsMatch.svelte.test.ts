...
const info = (over: Partial<RoomInfo> = {}): RoomInfo => ({
	...
	rulesVersion: 1,
	/*
	 * ЗЕРНО ВИБРАНЕ ТАК, ЩОБ ПЕРШИЙ ХІД БУВ У ГОСПОДАРЯ, і це не підгонка.
	 *
	 * Перший хід тепер виводиться із зерна (прохання автора: «перший ходить
	 * випадковий гравець»), а тести нижче — про ПЕРЕДАЧУ черги: промах передає,
	 * збіг лишає, позачерговий хід відкидають обидва. Мінливий перший хід зробив би
	 * кожен із них залежним від арифметики генератора — тобто зламався б не той
	 * інваріант, який вони стережуть.
	 *
	 * Саму випадковість перевіряє окремий пункт нижче, і саме він упаде, якщо вибір
	 * вернуть до сталого нуля.
	 */
	seed: 20260800,
	status: 'playing',
	hostUid: HOST,
...
describe('партія роздається з кімнати', () => {
	...
	it('перший хід той самий в обох, а не в того, хто натиснув', () => {
		const { host, guest, stop } = table();
		expect(host.actor?.id, 'на цьому зерні починає господар — див. коментар до зерна').toBe(HOST);
		expect(guest.actor?.id, 'клієнти розійшлися в тому, чия черга').toBe(host.actor?.id);
		expect(host.myTurn).toBe(true);
		expect(guest.myTurn).toBe(false);
		stop();
	});

	/**
	 * ПЕРШИЙ ХІД — ВІД ЗЕРНА, А НЕ ЗАВЖДИ ГОСПОДАР.
	 *
	 * Скарга автора: «перший ходить хост; очікуваний результат — перший ходить
	 * випадковий гравець». Доти в контролері стояв сталий нуль, а порядок гравців —
	 * за входом у кімнату.
	 *
	 * Два пункти в одному, і жоден не зайвий. Перший: вибір справді залежить від
	 * зерна — інакше «випадковість» була б словом у коментарі. Другий: на тому
	 * самому зерні він однаковий в обох — і це важливіше, бо саме тут ховається
	 * найгірший спосіб зробити це «правильно»: `Math.random()` дав би двом гравцям
	 * різну чергу з першої секунди, і дошки розійшлися б ще до першого ходу.
	 *
	 * Зворотні експерименти (AI-AGENT-PITFALLS-v8 § 1.1): вибір вернуто до `0` —
	 * червоніє «залежить від зерна»; `seededRandom(seed + 1)` замінено на
	 * `Math.random()` — червоніє «однаковий в обох». Обидва зроблені.
	 */
	it('перший хід залежить від зерна, і однаково в обох', () => {
		const starters = new Set<string | undefined>();
		for (const seed of [20260800, 20260817]) {
			const room = new LocalRoom(info({ seed }), members());
			const host = new PairsMatch(HOST, room.transport());
			const guest = new PairsMatch(GUEST, room.transport());
			const stop = [host.listen(), guest.listen()];

			expect(guest.actor?.id, `зерно ${seed}: клієнти розійшлися`).toBe(host.actor?.id);
			starters.add(host.actor?.id);
			stop.forEach((off) => off());
		}
		expect(starters.size, 'перший хід не залежить від зерна — він сталий').toBe(2);
	});
...
describe('суперник відпав', () => {
	...
	describe('відлік до автоматичного старту', () => {
		...
		/**
		 * РЕЖИМ КІМНАТИ — те, чого бракувало, і головна перевірка цього блока.
		 *
		 * Автор заміряв дефект так: натиск «не починати» скидав таймер, і той
		 * запускався сам. Причина була не в кнопці, а в тому, що відлік залежав
		 * ЛИШЕ від складу — двоє гравців означало «відлік мусить іти». Тому
		 * скасування жило один такт.
		 *
		 * Тут перевіряється саме та властивість, яку просив автор: після
		 * перемикання режиму позначка відліку зникає й НЕ вертається сама.
		 */
		it('перемикання режиму гасить відлік і тримає скасування', async () => {
			const { room, host, guest, stop } = table();
			await room.transport().setAutoStart(true);
			await room.transport().setCountdown(true);
			expect(host.countdownAt, 'відлік іде').not.toBeNull();

			// Це і є «не починати»: воно міняє РЕЖИМ, а не гасить таймер.
			await room.transport().setAutoStart(false);

			expect(host.autoStart, 'режим виключено').toBe(false);
			expect(guest.autoStart, 'і гість це бачить').toBe(false);
			expect(host.countdownAt, 'позначка відліку знята').toBeNull();
			expect(guest.countdownAt, 'і в гостя').toBeNull();
			stop();
		});

		/**
		 * Кімната, створена руками, САМА не починається.
		 *
		 * Відсутнє поле трактується як «виключено», і саме цей бік безпечніший:
		 * партія, що почалася сама там, де цього не просили, гірша за партію, яку
		 * треба почати кнопкою. Так само поводяться кімнати старших збірок.
		 */
		it('без поля режим виключений', () => {
			const { host, guest, stop } = table();
			expect(host.autoStart).toBe(false);
