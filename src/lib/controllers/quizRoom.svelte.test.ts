...
describe('адаптер вікторини', () => {
	...
	/**
	 * РЕВАНШ — НАСТУПНА ПАРТІЯ ТІЄЇ САМОЇ КОЛОДИ (прохання автора 2026-09-26): питання
	 * кімнати не повторюються, доки не поставлено всі (`config/quizDeck.ts`). Нове
	 * випадкове зерно стерло б облік кімнати, і повтори повернулися б.
	 *
	 * Зворотний експеримент: прибрати `rematchSeed` з адаптера — червоніє.
	 */
	it('реванш — наступна партія тієї самої колоди, а не нова колода', () => {
		const quiz = new QuizRoomState(() => 0.5);
		const seed = 123_456;
		expect(quiz.game.rematchSeed?.({ seed } as never)).toBe(seed + 2 ** 31);
	});
