...
export interface MemoryPlayer {
	...
	/**
	 * Імʼя з кімнати. Порожнє в соло — там гравця звуть ключем перекладу.
	 *
	 * Два поля, а не одне, бо це два різні джерела: «Ти» перекладається
	 * чотирма мовами, а «Alik» — ні, і проганяти чуже імʼя через словник
	 * означало б показати ключ замість імені.
	 */
	name?: string;
