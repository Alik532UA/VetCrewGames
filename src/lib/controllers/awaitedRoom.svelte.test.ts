...
describe('AwaitedRoom', () => {
	...
	describe('відповідь, яку вже не чекають', () => {
		 ...
		 *
		 * ДРУГИЙ `refresh()` стартує, коли перший уже ЧЕКАЄ ВІДПОВІДІ, а не разом із
		 * ним. Vitest 4.1.10 два одночасні `import()` ОДНОГО замоканого модуля віддає
		 * підміненим лише першому, другому — справжній модуль (відтворено окремо:
		 * три паралельні імпорти дають «мок, справжній, справжній»). Доти тут так і
		 * було: другий виклик ішов у справжній `net/ownRooms`, тобто в `connect()` і
		 * анонімну реєстрацію в живому Firebase, а тест проходив, бо порожній індекс
		 * нового акаунта давав той самий `[]`. Помітив це запобіжник мережі
		 * (`src/no-network.setup.ts`). Сценарій від цього не змінився: перша відповідь
		 * так само в дорозі, поки друга приходить і записується.
		 */
		it('повільніша відповідь не перебиває свіжішу', async () => {
			...
			await vi.waitFor(() => expect(listOwnRooms).toHaveBeenCalledTimes(1));
