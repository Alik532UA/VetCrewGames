...
export class PopulationGameController {
	 ...
	 *
	 * `force` — «час вийшов, зарахуй те, що стоїть».
	 *
	 * Умова повного ряду існує для ЛЮДИНИ: без неї один випадковий дотик по
	 * «Перевірити» витрачав би раунд на недороблену дошку. Але коли раунд і так
	 * закінчився, витрачати вже нічого — а гравець, який поставив дві картки з трьох
	 * і не встиг натиснути, інакше дістає нуль за те, що знав. Порожні місця йдуть
	 * як неправильні, і це не окрема арифметика: `slotResults` порівнює чисельність,
	 * а `null?.population` — це `undefined`, тобто не збіг.
	 *
	 * Дозвіл ЯВНИЙ, а не типовий, і саме тому це аргумент, а не знята умова: кнопка
	 * на дошці мусить лишитися вимкненою, доки ряд не повний.
	 */
	check(force = false): void {
		if ((!this.allSlotsFilled && !force) || this.checked) return;
		this.checked = true;
