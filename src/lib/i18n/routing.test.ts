 ...
 */
/**
 * Назва маршруту — лише ASCII.
 *
 * Кириличні гомогліфи («с» U+0441 замість «c», «е» U+0435 замість «e») дають
 * адресу, яка виглядає бездоганно й не працює: у шляху вона percent-кодується,
 * посилання, sitemap і `robots.txt` розходяться між собою, а в diff різниці не
 * видно взагалі. Назви маршрутів набирають руками — саме там таке й трапляється,
 * і саме тому це інваріант, а не уважність.
 */
describe('назви маршрутів', () => {
	it('складаються лише з ASCII', () => {
		const suspicious = Object.keys(LANGUAGE_ROUTES).filter((route) => /[^ -~]/.test(route));
		expect(
			suspicious.map(
				(route) => `${route} → ${[...route].map((c) => c.codePointAt(0)?.toString(16)).join(' ')}`
			),
			'нелатинський символ у назві маршруту — адреса percent-кодується й тихо розходиться з посиланнями'
		).toEqual([]);
	});
});
