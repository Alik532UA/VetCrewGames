...
describe('аватар гравця', () => {
	...
	/**
	 * ТИПОВИЙ АВАТАР — НЕ ВЛАСНИЙ, і саме це питає шапка.
	 *
	 * Плитка з типовим аватаром у шапці означала б «я вже щось вибрав», хоч людина
	 * або не вибирала, або вибрала саме типовий. В обох випадках правильний
	 * малюнок — звичайний значок акаунта.
	 */
	it('власним вважається лише вибір, що відрізняється від типового', async () => {
		const mine = await fresh();
		expect(mine.custom, 'нічого не вибрано').toBe(false);

		mine.set(DEFAULT_AVATAR);
		expect(mine.custom, 'вибрано типовий — у шапці лишається значок акаунта').toBe(false);
