 ...
 *
 * **Чому тут окремий `testTimeout`.** Бо це не юніт-перевірки, а прогони
 * симуляції, і типові 5000 мс vitest розраховані не на них. Ціна названа
 * числами: доба — це `TICKS_PER_DAY` (300) тактів, `raided()` шукає наліт до 200
 * спроб по 40 діб, а з патрулем налітають ушестеро рідше — тобто й шукати
 * доводиться вшестеро довше. Разом «засідка іноді коштує рейнджера» бере 915 мс
 * на цій машині під повним прогоном (`npm test`, 2026-08-20).
 *
 * Локальний запас у пʼять разів здавався достатнім — і не був: на двоядерному
 * раннері GitHub той самий тест вимикався за 5000 мс. Це сталося ДВІЧІ й обидва
 * рази валило деплой: прогін 32140191538 (18 серпня, 55 файлів) і 32341112824
 * (20 серпня, 63 файли) — той самий тест, та сама межа.
 *
 * Підняти межу, а не зрізати кількість прогонів, — свідомо. Допуски тут ±0,06
 * навколо часток 0,25 і 0,6; при 400 прогонах це вже ±2,8σ, тож менша вибірка
 * зробила б перевірку хиткою — тобто червоною випадково, а не по ділу. Дешевше
 * дати статистичній перевірці стільки часу, скільки вона справді бере.
 */
