...
/**
 * Найбільший вольєр, який узагалі буває.
 *
 * Пʼятдесят, а не десять: гра про АНТИзоопарк, і вольєр у ній — не клітка, а
 * шматок краєвиду. Розмір 50 — це 50×50 клітинок, тобто 110 світових одиниць у
 * поперек: слон із такою землею вже не «утримується», а живе.
 *
 * Перелічувати їх масивом більше немає сенсу: пʼятдесят кнопок не влазять на
 * жоден екран, і інтерфейс тепер дає лічильник із заготовками. Тому межа —
 * діапазон, а не список.
 */
export const MAX_ENCLOSURE_SIZE = 50;

/** Чи є таке число розміром вольєра. Ціле, від одного до межі. */
export const isEnclosureSize = (size: number): boolean =>
	Number.isInteger(size) && size >= 1 && size <= MAX_ENCLOSURE_SIZE;
