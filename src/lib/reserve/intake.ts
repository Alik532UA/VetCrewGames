...
export function intake(
	...
	/*
	 * Здоровʼя прибулої — КИДОК у межах походження, а не нуль для всіх.
	 *
	 * Доти будь-яка тварина приїжджала з нульовим одужанням, і походження
	 * впливало лише на ціну та репутацію. Тепер воно вирішує головне: куплена
	 * (75–100%) лікаря не потребує, врятована (1–50%) без нього помирає,
	 * а чорний ринок (10–100%) — лотерея, розіграна вже після оплати.
	 *
	 * Кидок іде ПЕРЕД тим, що вирішує придатність до випуску: порядок кидків
	 * частина стану, і міняти його місцями означало б розвернути кожен наявний
	 * сейв в інший світ.
	 */
	const health = terms.healthMin + roll(state) * (terms.healthMax - terms.healthMin);

	site.animals.push({
		id: state.nextAnimalId++,
		speciesId: species.id,
		origin: command.origin,
		// Прибула на повному здоровʼї вже здорова: тримати її «в лікуванні» до
		// першої доби означало б показувати стан, якого немає.
		stage: health >= 1 ? 'healthy' : 'recovering',
